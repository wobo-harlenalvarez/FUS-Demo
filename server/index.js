...
const genKey = (type, name) => `${type}:${name}`;
...
app.post('/', async (req, res) => {
    ...
    const savedFiles = JSON.parse(storedFiles || '[]');
...
app.get('/:refType', async (req, res) => {
    const files = JSON.parse(await redisClient.get('files') || '[]');
    const fileAtts = [];
    for (let filename of files) {
        if (!filename.startsWith(req.params.refType))
            continue;
        const storedFile = await redisClient.get(filename);
        const savedFile = JSON.parse(storedFile || '{}');
